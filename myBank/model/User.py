...
class User(object):
    ...
    def deleteUser():
        ...
        # TODO : demande toujours l'id meme si pas d'utilisateur créé
        if User.checkUsers():
            pass
        else:
            ...
            for user in users:
                if user:
                    print(f"L'utilisateur {user[1]} a été supprimé\n{CheckingAccount.separator}")
    ...
    def connect():
        ...
        #     pass
        # else:
        #     while not User.check(userPwd, userName):
        #         userName = input(f"Quel est votre nom d'utilisateur ?\n {CheckingAccount.separator}\n")
        #         userPwd = input(f"Quel est votre mot de passe ?\n {CheckingAccount.separator}\n")
        #         if not User.check(userPwd, userName):
        #             print(
        #                 f"{CheckingAccount.separator}\nVotre nom d'utilisateur ou votre mot de passe est faux\n {CheckingAccount.separator}")
        #             pass

        if User.checkUsers() is None:
            pass
        else:
            userName = input(f"Quel est votre nom d'utilisateur ?\n {CheckingAccount.separator}\n")
            userPwd = input(f"Quel est votre mot de passe ?\n {CheckingAccount.separator}\n")
        ...
        # userName = ""
        # userPwd = ""
        # # while not selectByUserName(userName):
        # userName = input(f"Quel est votre nom d'utilisateur ?\n {CheckingAccount.separator}\n")
        # userPwd = input(f"Quel est votre mot de passe ?\n {CheckingAccount.separator}\n")
        # user = selectByUserNameAndPwd(userName, userPwd).fetchall()
        # print(type(user))
        # print(user)
